  ...
  return (
    <AlertDialogPortal>
      ...
      <AlertDialogPrimitive.Popup
        ...
        className={cn(
          "fixed top-1/2 left-1/2 z-50 flex w-full max-w-sm -translate-x-1/2 -translate-y-1/2 flex-col gap-4 rounded-[var(--radius)] border border-border bg-surface-2 p-5 shadow-[0_8px_40px_rgba(0,0,0,0.45)] transition duration-150 ease-out data-ending-style:scale-95 data-ending-style:opacity-0 data-starting-style:scale-95 data-starting-style:opacity-0",
          className
        )}
        {...props}
      />
...
function AlertDialogAction({
  ...
  return (
    <AlertDialogPrimitive.Close
      ...
      render={<Button variant={variant} className={cn("sm:flex-1", className)} />}
      {...props}
    />
  )
}
...
function AlertDialogCancel({ className, ...props }: AlertDialogPrimitive.Close.Props) {
  return (
    <AlertDialogPrimitive.Close
      ...
      render={<Button variant="secondary" className={cn("sm:flex-1", className)} />}
      {...props}
    />
  )
}
